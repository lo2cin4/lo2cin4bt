...
import dash
from dash import html, dcc, callback_context as ctx, Input, Output, State, ALL
...
class ParameterIndexManager:
    ...
    def build_indexes(self):
        ...
        try:
            ...
            self.logger.info(f"參數索引建立完成，共 {len(self.strategy_indexes)} 個策略")
            return True
    ...
    def _build_strategy_parameter_indexes(self, strategy_key, indices):
        ...
        try:
            ...
            self.logger.info(f"策略 {strategy_key} 的參數索引建立完成，共 {len(self.parameter_combinations[strategy_key])} 個組合")
                                ...
                                for idx in data_indices:
                                    ...
                                    if 'Entry_params' in param:
                                        for ep in param['Entry_params']:
                                            if ep.get('indicator_type') == entry_param['indicator_type'] and ep.get('strat_idx') == entry_param['strat_idx']:
                                ...
                                for idx in data_indices:
                                    ...
                                    if 'Exit_params' in param:
                                        for ep in param['Exit_params']:
                                            if ep.get('indicator_type') == exit_param['indicator_type'] and ep.get('strat_idx') == exit_param['strat_idx']:
                                                values.add(ep.get(key))
                                if len(values) > 1:
                                    variable_params[param_name] = sorted(list(values))
...
class ParameterPlateauPlotter:
    ...
    def create_parameter_landscape_layout(self, data: Dict[str, Any]) -> html.Div:
        ...
        try:
            return html.Div([
                ...
                # 績效指標選擇區域
                html.Div([
                    html.Label("選擇績效指標:", className="form-label fw-bold mb-2"),
                    html.Div([
                        dbc.Button("Sharpe Ratio", id="btn-sharpe", color="primary", outline=False, className="me-2"),
                        dbc.Button("Sortino Ratio", id="btn-sortino", color="primary", outline=True, className="me-2"),
                        dbc.Button("Calmar Ratio", id="btn-calmar", color="primary", outline=True, className="me-2"),
                        dbc.Button("MDD", id="btn-mdd", color="primary", outline=True, className="me-2")
                    ], className="mb-3")
                ], className="mb-4"),
                ...
                html.Div([
                    ...
                    html.Div(id="parameter-control-panel", className="p-3 border rounded bg-light")
                ], className="mb-4"),
                
                # 參數高原圖表
                html.Div([
                    html.Label("參數高原圖表", className="form-label fw-bold"),
                    html.Div(id="parameter-landscape-chart", className="p-3 border rounded")
                ], className="mb-4")
    ...
    def create_parameter_control_panel(self, analysis: Dict[str, Any], strategy_key: str = None) -> html.Div:
        """
        創建參數控制面板 - 支持動態軸選擇
        ...
        try:
                ...
                if len(param_values) > 1:
                    ...
                    control = html.Div([
                        ...
                        html.Div([
                            ...
                            html.Label(f"{param_name}", className="form-label fw-bold ms-2")
                        ...
                        dcc.Slider(
                            ...
                            marks={val: str(val) for val in param_values},  # 使用實際參數值作為索引和標籤
            ...
            return html.Div([
                ...
                html.Div([
                    html.Div([
                        html.Label("固定參數🔓", className="form-label fw-bold me-2"),
                        dbc.Button(
                            "全選",
                    ...
                    html.P("勾選後滑動條變暗無法滑動，未勾選的參數可滑動選擇值", className="text-muted mb-0")
                ], className="d-flex align-items-center mb-2"),
                *param_controls,
                confirm_button
            ])
    ...
    def create_2d_parameter_heatmap(self, analysis: Dict[str, Any], metric: str, data: Dict[str, Any], 
                                   strategy_key: str = None, fixed_params: Dict[str, Any] = None) -> html.Div:
        """
        創建2D參數高原熱力圖 - 支持動態軸選擇
        ...
        try:
            ...
            if len(variable_params) < 2:
                ...
                return html.Div([
                    html.P("需要至少2個可變參數才能生成圖表", className="text-warning"),
                    html.P(f"📊 當前找到的可變參數: {len(variable_params)}", className="text-muted"),
                    html.P(f"🔍 參數詳情: {', '.join(param_info)}", className="text-muted"),
                    html.P("💡 建議: 檢查 Entry_params 和 Exit_params 中的參數變化", className="text-info")
                ])
            ...
            if fixed_params:
                ...
                if len(variable_axis_params) >= 2:
                    ...
                    return html.Div([
                        html.P("可變軸參數不足，無法生成2D圖表", className="text-warning"),
                        html.P(f"📊 當前可變軸參數: {len(variable_axis_params)}", className="text-muted"),
                        html.P(f"🔍 固定參數: {fixed_params}", className="text-info")
                    ])
            else:
                # 沒有固定參數時，使用前兩個參數作為軸
                if len(param_names) >= 2:
                    ...
                    return html.Div([
                        html.P("可變參數數量不足，無法生成2D圖表", className="text-warning"),
                        html.P(f"📊 當前可變參數: {len(param_names)}", className="text-muted")
                    ])
            ...
            for y_val in y_values:
                ...
                for x_val in x_values:
                    # 查找對應的績效指標值（支持固定參數篩選）
                    performance_value = self._find_performance_value_enhanced(analysis, x_axis, x_val, y_axis, y_val, metric, data, fixed_params)
            ...
            if valid_data_points == 0:
                return html.Div([
                    html.P(f"❌ 沒有找到有效的 {metric} 數據", className="text-warning"),
                    html.P(f"📊 數據統計: 總點數 {total_data_points}, 有效點數 {valid_data_points}, NaN 點數 {nan_data_points}", className="text-muted"),
                    html.P("💡 可能原因: 這些參數組合沒有產生交易，無法計算績效指標", className="text-info")
                ])
            ...
            # 獲取threshold標準和顏色映射
            colorscale = self._get_threshold_based_colorscale(metric, performance_matrix)
            ...
            fig = go.Figure(data=go.Heatmap(
                ...
                zmin=zmin,
                zmax=zmax,
                text=[[f"{val:.2f}" if val is not None and val != 0 else "" for val in row] for row in filtered_matrix],
                texttemplate="%{text}",
            ...
            if fixed_params:
                fixed_params_text = ", ".join([f"{k}={v}" for k, v in fixed_params.items()])
            ...
            info_panel = html.Div([
                ...
                html.P(f"有效數據點: {valid_data_points} ({valid_data_points/total_data_points*100:.1f}%)", className="mb-1 text-success"),
                html.P(f"無效數據點: {nan_data_points} ({nan_data_points/total_data_points*100:.1f}%)", className="mb-1 text-warning"),
                html.P(f"顯示的參數範圍: X軸({len(filtered_x_values)}個值), Y軸({len(filtered_y_values)}個值)", className="mb-1")
    ...
    def _find_performance_value_enhanced(self, analysis: Dict[str, Any], x_axis: str, x_val: str, y_axis: str, y_val: str, metric: str, data: Dict[str, Any], fixed_params: Dict[str, Any] = None) -> float:
        """
        增強的績效指標查找（支持固定參數篩選）
            ...
            for idx in parameter_indices:
                ...
                if fixed_params:
                    ...
                    for fixed_param_name, fixed_param_value in fixed_params.items():
                        if not self._check_param_match_enhanced(param, fixed_param_name, fixed_param_value):
        ...
        try:
            ...
            if 'Entry_params' in param:
                for entry_param in param['Entry_params']:
                    ...
                    for key, value in entry_param.items():
                        if key not in ['indicator_type', 'strat_idx']:
                            summary_parts.append(f"Entry_{indicator_type}{strat_idx}_{key}={value}")
            ...
            if 'Exit_params' in param:
                for exit_param in param['Exit_params']:
                    ...
                    for key, value in exit_param.items():
                        if key not in ['indicator_type', 'strat_idx']:
                            summary_parts.append(f"Exit_{indicator_type}{strat_idx}_{key}={value}")
    ...
    def _check_param_match_enhanced(self, param: Dict[str, Any], param_name: str, param_value: str) -> bool:
        """
        增強的參數匹配檢查（支持多指標策略）
        ...
        try:
            ...
            if param_name.startswith(('Entry_', 'Exit_')):
                ...
                parts = param_name.split('_', 2)  # 分割為 ['Entry/Exit', 'MA8/MA5', 'shortMA_period/longMA_period']
                if len(parts) >= 3:
                    ...
                    if param_key in param:
                        for param_item in param[param_key]:
                            # 檢查指標是否匹配
                            param_indicator = param_item.get('indicator_type', '') + param_item.get('strat_idx', '')
                            ...
                            try:
                                ...
                                value_match = abs(param_float - target_float) < 0.001  # 允許小的浮點誤差
                            except (ValueError, TypeError):
                                value_match = str(param_actual_value) == str(param_value)
    ...
    def register_callbacks(self, app: dash.Dash, data: Dict[str, Any]):
        ...
        def update_button_states(sharpe_clicks, sortino_clicks, calmar_clicks, mdd_clicks):
            """更新績效指標按鈕的長亮狀態"""
        ...
        def update_slider_states(checkbox_values):
            ...
            css_classes = ["slider-disabled" if val else "slider-enabled" for val in checkbox_values]
        ...
        def update_button_state(checkbox_values, strategy_key):
            ...
            # 構建現況顯示文字
            status_text = f"更新圖表 (已勾選: {checked_count}, 未勾選: {unchecked_count})"
        ...
        def update_parameter_control_panel(strategy_key):
            ...
            try:
                ...
                return html.P(f"更新參數控制面板失敗: {str(e)}", className="text-danger")
        ...
        def generate_parameter_landscape_chart(strategy_key, sharpe_clicks, sortino_clicks, calmar_clicks, mdd_clicks, update_clicks, 
            ...
            try:
                ...
                if checkbox_values and slider_values:
                    ...
                    for i, (is_checked, slider_value) in enumerate(zip(checkbox_values, slider_values)):
                        if i < len(param_names) and is_checked:
                            param_name = param_names[i]
                            fixed_params[param_name] = slider_value
                
                # 創建2D熱力圖（支持動態軸選擇）
                chart = self.create_2d_parameter_heatmap(analysis, metric, data, strategy_key, fixed_params)
                return chart
                
            except Exception as e:
                return html.P(f"生成圖表失敗: {str(e)}", className="text-danger")

    def _get_threshold_based_colorscale(self, metric: str, performance_matrix: List[List[float]]) -> List[List]:
        """
        ...
        try:
            ...
            if not valid_values:
                # 如果沒有有效值，返回預設顏色
                return [
                    [0.0, '#520032'], [0.5, '#F2933A'], [1.0, '#F9F8BB']
                ]
            ...
            if metric == "Max_drawdown":
                ...
                if min_val >= thresholds['excellent']:
                    # 所有值都在優秀範圍內（>= -0.05，即 <= 5%）
                    colorscale = [
                        [0.0, '#F9F8BB'], [0.5, '#FFF399'], [1.0, '#FFE252']
                    ]
                elif min_val >= thresholds['good']:
                    # 最低到良好範圍（>= -0.10，即 <= 10%）
                    colorscale = [
                        [0.0, '#FFD525'], [0.5, '#FFE252'], [1.0, '#FFF399']
                    ]
                elif min_val >= thresholds['qualified']:
                    # 最低到合格範圍（>= -0.20，即 <= 20%）
                    colorscale = [
                        [0.0, '#F2933A'], [0.5, '#F5A23A'], [1.0, '#FFD525']
                    ]
                else:
                    # 包含不可接受範圍（< -0.20，即 > 20%）
                    colorscale = [
                        [0.0, '#520032'], [0.3, '#751614'], [0.6, '#F2933A'], [1.0, '#F0AA38']
                    ]
            else:
