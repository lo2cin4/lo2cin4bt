    ...
    def _get_threshold_based_colorscale(
        ...
        try:
            ...
            if metric == "Max_drawdown":
                ...
                # 使用固定的顏色漸變，基於threshold標準映射到zmin(-0.7)和zmax(-0.0)
                # 顏色漸變位置對應：
                # 0.0 (zmin=-0.7) -> 深紫色（最差）
                # 0.14 (threshold=-0.6) -> 深紅色
                # 0.29 (threshold=-0.5) -> 橙色
                # 0.43 (threshold=-0.3) -> 黃色
                # 0.86 (threshold=-0.1) -> 淺黃色
                # 1.0 (zmax=-0.0) -> 最淺黃色（最好）
                colorscale = [
                    [0.0, "#520032"],      # -0.7 (最差，深紫色)
                    [0.14, "#751614"],     # -0.6 (深紅色)
                    [0.29, "#8B1A1A"],     # -0.5 (暗紅色)
                    [0.43, "#F2933A"],     # -0.3 (橙色)
                    [0.57, "#FFD525"],     # -0.2 (黃色)
                    [0.71, "#FFE252"],     # -0.1 (淺黃色)
                    [0.86, "#FFF399"],     # -0.05 (更淺黃色)
                    [1.0, "#F9F8BB"],      # -0.0 (最好，最淺黃色)
                ]
            else:
