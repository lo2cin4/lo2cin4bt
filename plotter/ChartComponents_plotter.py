...
【功能說明】
------------------------------------------------------------
本檔案為 plotter 模組的圖表組件生成器，負責生成各種圖表組件。
包括權益曲線圖、績效比較圖、參數分布圖等。

【關聯流程與數據流】
------------------------------------------------------------
- 主流程：接收數據 → 處理數據 → 生成圖表 → 返回組件
- 各模組間數據流明確，流程如下：
...
- ChartComponents 負責數據處理、圖表配置、樣式設置
- **每次新增/修改圖表類型、樣式時，必須同步檢查本檔案與所有依賴模組**
...
【維護重點】
------------------------------------------------------------
- 新增/修改圖表類型、樣式時，務必同步更新本檔案與所有依賴模組
- 圖表配置和樣式設置需要特別注意一致性
