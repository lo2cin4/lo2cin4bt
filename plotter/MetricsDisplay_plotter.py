...
【功能說明】
------------------------------------------------------------
本檔案為 plotter 模組的績效指標顯示組件，負責生成績效指標表格和詳情顯示。
包括表格生成、指標格式化、詳情卡片等。

【關聯流程與數據流】
------------------------------------------------------------
- 主流程：接收指標數據 → 格式化處理 → 生成組件 → 返回顯示
- 各模組間數據流明確，流程如下：
...
- MetricsDisplay 負責數據格式化、表格生成、詳情顯示
- **每次新增/修改指標類型、顯示格式時，必須同步檢查本檔案與所有依賴模組**
...
【維護重點】
------------------------------------------------------------
- 新增/修改指標類型、顯示格式時，務必同步更新本檔案與所有依賴模組
- 指標格式化和表格生成需要特別注意一致性
...
class MetricsDisplay:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self.metric_categories = {
            ...
            'trading': ['total_trades', 'win_rate', 'profit_factor', 'avg_trade_return'],
            'bah_comparison': ['bah_return', 'bah_annualized_return', 'bah_max_drawdown', 'excess_return']
        }
    ...
    def create_metrics_table(self, metrics_data: Dict[str, Any], 
                           ...
                           sort_by: str = 'net_profit') -> html.Div:
        """
        創建績效指標表格
        ...
        try:
                ...
                if param_key in metrics_data:
                    ...
                    for metric_key, metric_name in self.metric_names.items():
                        if metric_key in metric_data:
                            ...
                            row[metric_name] = self._format_metric_value(metric_key, value)
                        else:
                            row[metric_name] = "N/A"
            ...
            if sort_by in self.metric_names:
                ...
                if sort_column in table_data[0]:
                    table_data.sort(key=lambda x: self._extract_numeric_value(x.get(sort_column, 0)), reverse=True)
            ...
            return html.Div([
                html.H6(f"績效指標表格 (按 {self.metric_names.get(sort_by, sort_by)} 排序)", 
                       className="mb-3"),
                table
            ])
    ...
    def create_detailed_metrics_card(self, param_key: str, 
                                   metrics_data: Dict[str, Any],
                                   parameters: Dict[str, Any]) -> html.Div:
        """
        創建詳細績效指標卡片
            ...
            for category, metric_keys in self.metric_categories.items():
                ...
                for metric_key in metric_keys:
                    if metric_key in metrics:
                        ...
                        card = dbc.Card([
                            dbc.CardBody([
                                html.H6(self.metric_names.get(metric_key, metric_key), 
                ...
                if category_cards:
                    ...
                    cards.append(html.Div([
                        ...
                        dbc.Row([dbc.Col(card, width=6) for card in category_cards])
                    ]))
    ...
    def create_summary_statistics(self, metrics_data: Dict[str, Any], 
                                selected_params: List[str]) -> html.Div:
        """
        創建摘要統計信息
        ...
        try:
            ...
            for metric_key in self.metric_names.keys():
                values = []
                for param_key in selected_params:
                    if param_key in metrics_data and metric_key in metrics_data[param_key]:
            ...
            return html.Div([
                html.H6(f"摘要統計 (共 {len(selected_params)} 個組合)", className="mb-3"),
                table
            ])
