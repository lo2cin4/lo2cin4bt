...
【功能說明】
------------------------------------------------------------
本模組為 Lo2cin4BT 回測框架的「移動平均線指標」模組，負責產生各種 MA（SMA/EMA/WMA）指標序列與信號，支援單均線、雙均線及多種策略型態，並可根據預測因子產生交易信號。

【關聯流程與數據流】
------------------------------------------------------------
- 由 IndicatorsBacktester 調用，根據參數產生 MA 信號
- 產生信號傳遞給 BacktestEngine 進行回測
- 主要數據流：

```mermaid
flowchart TD
    A[IndicatorsBacktester] -->|調用| B[MovingAverage_Indicator_backtester]
...
【主控流程細節】
------------------------------------------------------------
- get_params() 產生所有策略型態與參數組合，支援單/雙均線、連續突破等
- generate_signals() 根據參數與預測因子產生交易信號，支援多種策略型態
- calculate() 支援 SMA、EMA、WMA 等多種均線計算
- 支援細分型態（如 MA1~MA12）與多種參數配置

【維護與擴充提醒】
------------------------------------------------------------
- 新增策略型態、參數時，請同步更新 get_params/MA_DESCRIPTIONS/頂部註解
...
- 預測因子欄位不存在或數據對齊錯誤
- 信號產生邏輯未考慮 NaN 或邊界情況
- SMA/EMA/WMA 計算方式未正確對應參數

【範例】
------------------------------------------------------------
- 產生參數組合：MovingAverageIndicator.get_params(strat_idx, params_config)
- 產生信號：MovingAverageIndicator(data, params).generate_signals(predictor)
