...
if NUMBA_AVAILABLE:
    ...
    def _generate_hl_signals_njit(predictor_values, n_length, m_length, strat_idx):
        """
        ...
        for i in range(m_length + n_length - 1, n):
            ...
            historical_high = predictor_values[start_idx:end_idx].max()
            historical_low = predictor_values[start_idx:end_idx].min()
...
class HLIndicator:
    ...
    def get_strategy_descriptions():
        # 回傳 dict: {'HL{i+1}': '描述', ...}
        return {f"HL{i+1}": desc for i, desc in enumerate(HLIndicator.STRATEGY_DESCRIPTIONS)}
    ...
    def generate_signals(self, predictor=None):
        ...
        # 檢查數據是否足夠
        min_required_length = m_length + n_length - 1  # 需要m_length + n_length - 1天的數據
        if len(self.data) < min_required_length:
            raise ValueError(f"數據長度不足：需要至少{min_required_length}個數據點，當前只有{len(self.data)}個")
    ...
    @staticmethod
    def vectorized_calculate_hl_signals(tasks, predictor, signals_matrix, global_hl_cache=None, data=None):
        """
        ...
        for i, (n_length, m_length, strat_idx, task_idx, indicator_idx) in enumerate(zip(n_lengths, m_lengths, strat_indices, task_indices, indicator_indices)):
            try:
                ...
                if hasattr(HLIndicator, 'logger'):
                    HLIndicator.logger.warning(f"HL突破信號生成失敗 (task_idx={task_idx}, indicator_idx={indicator_idx}): {e}")
