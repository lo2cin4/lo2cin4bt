...
if NUMBA_AVAILABLE:
    ...
    @njit(fastmath=True, cache=True)
    def _generate_percentile_signals_njit(predictor_values, percentile_values, strat_idx, window, m1=None, m2=None):
        """
        使用 Numba 生成百分位信號
...
class PercentileIndicator:
    ...
    def get_strategy_descriptions():
        """獲取策略描述字典"""
        return {f"PERC{i+1}": desc for i, desc in enumerate(PercentileIndicator.STRATEGY_DESCRIPTIONS)}
        ...
        if strat_idx is None:
            if "strat_idx" in params_config:
                ...
                if "alias" in params_config:
                    ...
                    raise ValueError("strat_idx 必須由 UserInterface 提供，或存在於 params_config 中")
    ...
    def generate_signals(self, predictor=None):
        ...
        if NUMBA_AVAILABLE:
            ...
            if strat_idx in [1, 2, 3, 4]:
                ...
                # 計算滾動百分位
                percentile_values = _calculate_rolling_percentile_njit(predictor_values, window, percentile)
                ...
                # 計算兩個百分位值
                percentile1_values = _calculate_rolling_percentile_njit(predictor_values, window, m1)
                percentile2_values = _calculate_rolling_percentile_njit(predictor_values, window, m2)
    ...
    @staticmethod
    def vectorized_calculate_percentile_signals(tasks, predictor, signals_matrix, global_percentile_cache=None, data=None):
        """
            ...
            if window <= len(data):
                ...
                if cache_key not in global_percentile_cache:
                    ...
                    if NUMBA_AVAILABLE:
                        ...
                        percentile_values = data[predictor].rolling(window=window).quantile(percentile / 100.0).values
                        percentile_values = np.nan_to_num(percentile_values, nan=0.0)
                    
                    global_percentile_cache[cache_key] = percentile_values
        
        # 為每個任務生成信號 - 使用Numba優化
        for i, (window, percentile, strat_idx, task_idx, indicator_idx) in enumerate(zip(windows, percentiles, strat_indices, task_indices, indicator_indices)):
            try:
                if strat_idx in [1, 2, 3, 4]:
                    # 單百分位策略
                    cache_key = (window, percentile, predictor)
                    ...
                    # 雙百分位區間策略
                    m1 = m1_values[i - len([x for x in strat_indices[:i] if x in [1, 2, 3, 4]])]
                    m2 = m2_values[i - len([x for x in strat_indices[:i] if x in [1, 2, 3, 4]])]
                    ...
                    if cache_key1 not in global_percentile_cache:
                        if NUMBA_AVAILABLE:
                            global_percentile_cache[cache_key1] = _calculate_rolling_percentile_njit(predictor_values, window, m1)
                        else:
                            global_percentile_cache[cache_key1] = data[predictor].rolling(window=window).quantile(m1 / 100.0).values
                    
                    if cache_key2 not in global_percentile_cache:
                        if NUMBA_AVAILABLE:
                            global_percentile_cache[cache_key2] = _calculate_rolling_percentile_njit(predictor_values, window, m2)
                        else:
                            global_percentile_cache[cache_key2] = data[predictor].rolling(window=window).quantile(m2 / 100.0).values
                ...
                if hasattr(PercentileIndicator, 'logger'):
                    PercentileIndicator.logger.warning(f"百分位信號生成失敗 (task_idx={task_idx}, indicator_idx={indicator_idx}): {e}")
