...
class ProgressMonitor:
    ...
    def finish(self):
        ...
        if self.progress is not None and self.task is not None:
            ...
            self.progress.update(self.task, description=f"生成回測結果 ({self.total_batches}/{self.total_batches} 批次, {self.total_backtests}/{self.total_backtests} 任務)")
...
class VectorBacktestEngine:
    ...
    def _precompute_data(self):
        ...
        if len(self._price_cache['Close']) > 1:
            self._price_cache['returns'][1:] = np.diff(self._price_cache['Close']) / self._price_cache['Close'][:-1]
    ...
    def run_backtests(self, config: Dict) -> List[Dict]:
        ...
        console.print(Panel(f"將執行向量化回測：{len(all_combinations)} 種參數組合 x {len(predictors)} 個預測因子 = {total_backtests} 次回測\n交易參數：{trading_params}", 
                           title="[bold #8f1511]🚀 向量化回測引擎[/bold #8f1511]", border_style="#dbac30"))
        ...
        while True:
            ...
            if confirm in ['', 'y', 'yes']:
                ...
                console.print(Panel("已取消回測", title="[bold #8f1511]❌ 取消[/bold #8f1511]", border_style="#8f1511"))
                return []
        ...
        # 預先收集配置信息
        config_info = SpecMonitor.collect_config_info(len(all_combinations) * len(predictors))
        
        # 創建配置信息面板
        config_panel_content = f"🚀 開始向量化回測...\n📊 初始記憶體使用: {initial_memory:.1f} MB"
        
        console.print(Panel(config_panel_content, 
                           title="[bold #dbac30]⚡ 向量化性能監控[/bold #dbac30]", border_style="#dbac30"))
        ...
        if memory_used > warning_threshold:
            memory_percent = (memory_used / (memory_thresholds['total_memory_gb'] * 1024)) * 100 if memory_thresholds['total_memory_gb'] > 0 else 0
            console.print(Panel(f"⚠️ 記憶體使用過高: {memory_used:.1f} MB ({memory_percent:.1f}% of {memory_thresholds['total_memory_gb']:.1f}GB)，強制垃圾回收", 
        ...
        for r in all_results:
            if r.get("error") is None:
                ...
                if len(records) == 0:
                    zero_trade_count += 1
                    trade_count_details.append(f"無記錄: {r.get('Backtest_id', 'unknown')}")
                elif (records['Trade_action'] == 1).sum() == 0:
                    zero_trade_count += 1
                    trade_count_details.append(f"無開倉: {r.get('Backtest_id', 'unknown')} (記錄數: {len(records)})")
                else:
                    trade_count_details.append(f"有交易: {r.get('Backtest_id', 'unknown')} (開倉數: {(records['Trade_action'] == 1).sum()})")
        ...
        if zero_trade_count > 0:
            ...
            for r in all_results:
                if r.get("error") is None:
                    records = r.get("records", pd.DataFrame())
                    if len(records) == 0 or (len(records) > 0 and (records['Trade_action'] == 1).sum() == 0):
                        sample_no_trade = r
                        break
...
"""
        ...
        console.print(Panel(summary_text, title="[bold #dbac30]🎯 向量化回測結果[/bold #dbac30]", border_style="#dbac30"))
    ...
    def _true_vectorized_backtest(self, all_combinations: List[Tuple], condition_pairs: List[Dict], 
        ...
        with parallel_progress:
            ...
            # 步驟3: 向量化交易模擬
            all_trade_results = self._simulate_all_trades_vectorized(all_signals, trading_params)
            
            # 步驟4: 並行結果生成（包含所有 print 語句）
            all_results = self._generate_all_results_vectorized(all_tasks, all_trade_results, all_signals, condition_pairs, parallel_progress, None, total_backtests)
            
        return all_results
    
    def _generate_all_tasks_matrix(self, all_combinations: List[Tuple], predictors: List[str]) -> Dict:
        """一次性生成所有任務矩陣"""
        all_tasks = {
            'combinations': [],
            'predictors': [],
    ...
    def _generate_all_signals_vectorized(self, all_tasks: Dict, condition_pairs: List[Dict]) -> Dict:
        ...
        for group_idx, group in enumerate(strategy_groups):
            from rich.console import Console
            console = Console()
            console.print(Panel(f"🔧 處理策略分組 {group_idx + 1}/{len(strategy_groups)}: {len(group['tasks'])} 個任務", 
    ...
    def _group_strategies_by_indicator_count(self, all_tasks: Dict, condition_pairs: List[Dict]) -> List[Dict]:
        """按指標數量分組策略"""
        groups = {}
    ...
    def _process_strategy_group(self, group: Dict) -> Dict:
        ...
        for task_info in tasks:
            ...
            strategy_idx = task_info['strategy_idx']
        ...
        from rich.console import Console
        console = Console()
        console.print(Panel(f"🚀 生成信號: {len(entry_params_list)} 任務, 開倉指標: {entry_count}, 平倉指標: {exit_count}", 
                           title=Text("👨‍💻 交易回測 Backtester", style="bold #8f1511"), border_style="#dbac30"))
        entry_signals_matrix = self._vectorized_generate_signals(entry_params_list, predictors_list)
        exit_signals_matrix = self._vectorized_generate_signals(exit_params_list, predictors_list)
        ...
        # 處理 NDayCycle 平倉信號
        combined_exit_signals = NDayCycleIndicator.process_ndaycycle_exit_signals_in_batch(
            exit_params_list, combined_entry_signals, combined_exit_signals, self.data
        )
    ...
    def _simulate_all_trades_vectorized(self, all_signals: Dict, trading_params: Dict) -> Dict:
        """向量化交易模擬 - 處理分組後的信號"""
        ...
        if isinstance(all_signals['entry_signals'], list):
            ...
            # 合併所有分組的信號
            entry_signals = np.column_stack(entry_signals_list) if entry_signals_list else np.zeros((len(self.data), 0))
            exit_signals = np.column_stack(exit_signals_list) if exit_signals_list else np.zeros((len(self.data), 0))
        else:
            # 單個numpy數組（未分組）
        ...
        simulator = TradeSimulator_backtester(
            self.data,
            pd.Series(entry_signals[:, 0]) if entry_signals.shape[1] > 0 else pd.Series(0, index=self.data.index),  # 使用第一個策略的信號作為示例
            pd.Series(exit_signals[:, 0]) if exit_signals.shape[1] > 0 else pd.Series(0, index=self.data.index),
        ...
        trade_results = simulator.simulate_trades_vectorized(entry_signals, exit_signals, trading_params)
        
        return trade_results
    
    # 舊的批次處理函數已被 _process_batch_results_optimized 取代

    def _generate_all_results_vectorized(self, all_tasks: Dict, all_trade_results: Dict, all_signals: Dict, condition_pairs: List[Dict], progress=None, task=None, total_backtests=None) -> List[Dict]:
        ...
        from rich.console import Console
        console = Console()
        console.print(Panel(f"🔧 並行處理模式: {n_tasks} 個任務, {n_cores} 核心", 
                           title=Text("👨‍💻 交易回測 Backtester", style="bold #8f1511"), border_style="#dbac30"))
        ...
        console.print(Panel(f"🔧 批次配置: {len(batch_indices)} 個批次, 每批次約 {batch_size} 個任務", 
                           title=Text("👨‍💻 交易回測 Backtester", style="bold #8f1511"), border_style="#dbac30"))
        ...
        if progress is not None:
            ...
            progress_monitor = ProgressMonitor(progress, task, total_backtests, len(batch_indices))
            progress_monitor.set_batch_sizes(batch_sizes)
        
        # 智能選擇處理模式
        if len(batch_indices) == 1:
            # 單批次直接處理，避免多進程開銷
            console.print(Panel(f"🔧 單進程處理: {n_tasks} 個任務", 
                               title=Text("👨‍💻 交易回測 Backtester", style="bold #8f1511"), border_style="#dbac30"))
            
            batch_data = self._prepare_batch_data(batch_indices[0], all_tasks, all_trade_results, all_signals, condition_pairs)
            ...
            if memory_used > warning_threshold:  # 使用動態閾值
                memory_percent = (memory_used / (memory_thresholds['total_memory_gb'] * 1024)) * 100 if memory_thresholds['total_memory_gb'] > 0 else 0
                console.print(Panel(f"⚠️ 記憶體使用過高: {memory_used:.1f} MB ({memory_percent:.1f}% of {memory_thresholds['total_memory_gb']:.1f}GB)，強制垃圾回收", 
                                   title=Text("💾 記憶體管理", style="bold #8f1511"), border_style="#dbac30"))
                gc.collect()
        else:
            # 多批次並行處理
            results = []
            try:
                with ProcessPoolExecutor(max_workers=n_cores) as executor:
                    ...
                    for batch_idx, batch_idx_list in enumerate(batch_indices):
                        batch_data = self._prepare_batch_data(batch_idx_list, all_tasks, all_trade_results, all_signals, condition_pairs)
                        future = executor.submit(self._process_batch_results_optimized, batch_data)
                ...
                for batch_idx, future in futures:
                    try:
                        ...
                        # 通知進度監控器批次完成
                        if progress_monitor is not None:
                            progress_monitor.batch_completed(batch_idx=batch_idx, completed_tasks_in_batch=len(batch_results))
                        ...
                        console.print(Panel(f"批次 {batch_idx+1} 處理失敗: {batch_error}", 
                                           title=Text("⚠️ 處理錯誤", style="bold #8f1511"), border_style="#dbac30"))
                        
                        # 為失敗的批次添加錯誤結果
                        batch_size = len(batch_indices[batch_idx]) if batch_idx < len(batch_indices) else 1
                        ...
                        # 通知進度監控器批次完成（即使失敗）
                        if progress_monitor is not None:
                            progress_monitor.batch_completed(batch_idx=batch_idx, completed_tasks_in_batch=batch_size)
                ...
                console.print(Panel(f"並行處理失敗: {e}", 
                                   title=Text("❌ 處理錯誤", style="bold #8f1511"), border_style="#dbac30"))
                # 如果並行處理完全失敗，使用簡化的串行處理
                console.print(Panel("回退到簡化串行處理...", 
                                   title="⚠️ 處理警告", border_style="#8f1511"))
                return self._generate_all_results_simple(all_tasks, all_trade_results, all_signals, condition_pairs, progress, task, total_backtests)
        
        return results

    def _prepare_batch_data(self, batch_indices: List[int], all_tasks: Dict, all_trade_results: Dict, all_signals: Dict, condition_pairs: List[Dict]) -> Dict:
        ...
        if isinstance(all_signals['entry_signals'], list):
            ...
            for group_signals in all_signals['entry_signals']:
                group_size = group_signals.shape[1] if len(group_signals.shape) > 1 else 1
                if len(group_signals.shape) > 1:
                    entry_signals_full[:, current_task_idx:current_task_idx + group_size] = group_signals
                else:
            ...
            for group_signals in all_signals['exit_signals']:
                group_size = group_signals.shape[1] if len(group_signals.shape) > 1 else 1
                if len(group_signals.shape) > 1:
                    exit_signals_full[:, current_task_idx:current_task_idx + group_size] = group_signals
                else:
    ...
    def _process_batch_results_optimized(self, batch_data: Dict) -> List[Dict]:
        ...
        try:
            ...
            batch_idx_map = {task_idx: idx for idx, task_idx in enumerate(batch_indices)}
            ...
            for task_idx in batch_indices:
                try:
                    ...
                    error_result = {
                        ...
                        'strategy_id': f"strategy_{strategy_idx+1}" if 'strategy_idx' in locals() else "unknown",
                        'params': {
                            'entry': [],
                            'exit': [],
    ...
    def _generate_all_results_simple(self, all_tasks: Dict, all_trade_results: Dict, all_signals: Dict, condition_pairs: List[Dict], progress=None, task=None, total_backtests=None) -> List[Dict]:
        """簡化的串行處理（備用方案）- 改進進度追蹤"""
        import os
        import multiprocessing
        ...
        from rich.console import Console
        console = Console()
        console.print(Panel(f"🔧 簡化串行處理: {n_tasks} 個任務", 
                           title=Text("👨‍💻 交易回測 Backtester", style="bold #8f1511"), border_style="#dbac30"))
        ...
        if progress is not None:
            ...
            # 初始化進度條顯示
            progress.update(task, description=f"生成回測結果 (0/{n_tasks} 批次, 0/{total_backtests} 任務)")
        ...
        for task_idx in range(n_tasks):
            try:
                ...
                result = self._generate_single_result(
                    task_idx,
                    entry_signals[:, task_idx] if task_idx < entry_signals.shape[1] else np.zeros(len(self.data)),
                    exit_signals[:, task_idx] if task_idx < exit_signals.shape[1] else np.zeros(len(self.data)),
                ...
                # 通知進度監控器任務完成
                if progress_monitor is not None:
                    progress_monitor.batch_completed(batch_idx=task_idx, completed_tasks_in_batch=1)
                ...
                error_result = {
                    ...
                    'params': {
                        'entry': [],
                        'exit': [],
                        'predictor': all_tasks['predictors'][task_idx] if task_idx < len(all_tasks['predictors']) else "unknown"
                    },
                    'records': pd.DataFrame(),
                    'warning_msg': None,
                    'error': error_msg
        ...
        console.print(Panel(f"串行處理完成，總共返回 {len(results)} 個結果", 
                           title=Text("👨‍💻 交易回測 Backtester", style="bold #8f1511"), border_style="#dbac30"))
        
        return results
    
    def _generate_single_result(self, task_idx: int, entry_signal: np.ndarray, exit_signal: np.ndarray,
                               position: np.ndarray, returns: np.ndarray, trade_actions: np.ndarray,
                               equity_values: np.ndarray, predictor: str, backtest_id: str, 
                               entry_params: List, exit_params: List, trading_params: Dict) -> Dict:
        ...
        result = simulator.generate_single_result(
            task_idx, entry_signal, exit_signal, position, returns, trade_actions, equity_values,
            predictor, backtest_id, entry_params, exit_params, trading_params
        )
    ...
    def _vectorized_generate_signals(self, params_list: List[List], predictors: List[str]) -> np.ndarray:
    ...
    def _vectorized_combine_signals(self, signals_matrix: np.ndarray, is_exit_signals: bool = False) -> np.ndarray:
        """向量化合併信號"""
        if NUMBA_AVAILABLE:
    ...
    def _vectorized_trade_simulation(self, entry_signals: np.ndarray, exit_signals: np.ndarray,
                                   predictors: List[str], backtest_ids: List[str],
                                   entry_params_list: List[List], exit_params_list: List[List],
                                   trading_params: Dict) -> List[Dict]:
        """向量化交易模擬"""
        ...
        if NUMBA_AVAILABLE:
            positions, returns, trade_actions, equity_values = _vectorized_trade_simulation_njit(
                entry_signals, exit_signals, price_returns,
                trading_params['transaction_cost'], trading_params['slippage']
            )
        else:
            # 備用實現
            positions, returns, trade_actions, equity_values = self._fallback_trade_simulation(
        ...
        for task_idx in range(n_tasks):
            result = self._generate_single_result(
                task_idx, entry_signals[:, task_idx], exit_signals[:, task_idx],
                positions[:, task_idx], returns[:, task_idx], trade_actions[:, task_idx],
                equity_values[:, task_idx], predictors[task_idx], backtest_ids[task_idx],
                entry_params_list[task_idx], exit_params_list[task_idx],
                trading_params
            )
            results.append(result)
