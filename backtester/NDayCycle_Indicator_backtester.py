...
class NDayCycleIndicator(BaseIndicator):
    ...
    def generate_signals(self, params: Dict[str, Any], predictor: str = 'Close') -> pd.Series:
        """
        生成交易信號
    ...
    @staticmethod
    def generate_exit_signal_from_entry(entry_signal: pd.Series, n: int, strat_idx: int) -> pd.Series:
        """
        根據開倉信號生成平倉信號
        ...
        """
        if NUMBA_AVAILABLE:
            exit_signals = _generate_exit_signal_njit(
                entry_signal.values, n, strat_idx
            )
        else:
    ...
    @staticmethod
    def _generate_exit_signal_fallback(entry_signal: pd.Series, n: int, strat_idx: int) -> np.ndarray:
        """備用的平倉信號生成"""
    ...
    @staticmethod
    def vectorized_calculate_ndaycycle_signals(tasks: List[Tuple], predictor: str, 
                                             signals_matrix: np.ndarray, data: pd.DataFrame) -> None:
        """
    ...
    @staticmethod
    def handle_ndaycycle_exit_signals(exit_params_list: List, 
        ...
        for task_idx, exit_params in enumerate(exit_params_list):
            ...
            if isinstance(exit_params, list) and len(exit_params) > 0:
                ...
                if (hasattr(exit_param, 'indicator_type') and 
                    exit_param.indicator_type == 'NDayCycle'):
                    ...
                    if n is not None and strat_idx is not None:
                        # 使用合併後的開倉信號來生成平倉信號
                        entry_signal_series = pd.Series(entry_signals[:, task_idx], index=data.index)
                        
                        # 檢查是否有開倉信號
                        if np.count_nonzero(entry_signal_series.values) > 0:
                            exit_signal_series = NDayCycleIndicator.generate_exit_signal_from_entry(
    ...
    @staticmethod
    def process_ndaycycle_exit_signals_in_batch(exit_params_list: List, 
                                              entry_signals: np.ndarray, 
                                              exit_signals: np.ndarray,
                                              data: pd.DataFrame) -> np.ndarray:
        """
...
if NUMBA_AVAILABLE:
    @njit(fastmath=True)
    def _calculate_nday_signals_njit(close_prices: np.ndarray, n: int, strat_idx: int) -> np.ndarray:
    ...
    @njit(fastmath=True)
    def _generate_exit_signal_njit(entry_signals: np.ndarray, n: int, strat_idx: int) -> np.ndarray:
        """Numba 優化的平倉信號生成"""
